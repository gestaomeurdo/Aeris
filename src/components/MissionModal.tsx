...
import { X, Headphones, FileText, Play, Pause, ChevronLeft, Video, BookOpen, Database, Radio, Activity, Volume2 } from 'lucide-react';
                  ...
                  <div className="relative w-full h-full flex items-center justify-center p-8 md:p-20">
                    ...
                    <div className="relative z-10 w-full max-w-4xl flex flex-col md:flex-row items-center gap-12 bg-black/40 backdrop-blur-xl p-10 md:p-16 rounded-[40px] border border-white/10 shadow-2xl">
                      <div className="relative w-48 h-48 md:w-64 md:h-64 flex-shrink-0 group">
                        <img src={coverUrl} className="w-full h-full object-cover rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] group-hover:scale-105 transition-transform duration-700" alt="Cover" />
                        <div className="absolute inset-0 rounded-3xl border-2 border-[#00E5FF]/20 group-hover:border-[#00E5FF]/50 transition-colors" />
                        {(isPlayingPodcast || isPlayingAudiobook) && (
                          <div className="absolute -bottom-4 -right-4 bg-[#00E5FF] p-3 rounded-2xl shadow-[0_0_20px_#00E5FF]">
                      ...
                      <div className="flex-1 space-y-8 text-center md:text-left">
                        <div className="space-y-2">
                          <span className="text-xs font-mono font-black text-[#00E5FF] uppercase tracking-[0.4em]">{isPlayingAudiobook ? 'AUDIOBOOK' : 'PODCAST'} UPLINK ACTIVE</span>
                          <h3 className="text-3xl md:text-5xl font-black text-white uppercase tracking-tighter leading-tight">{module.title}</h3>
                          <p className="text-sm text-white/40 font-medium">{module.desc}</p>
                        </div>
                        ...
                        <div className="space-y-4">
                           ...
                           <div className="flex items-center justify-center md:justify-start gap-8">
                              <button 
                                ...
                                className="w-20 h-20 bg-[#00E5FF] text-black rounded-full flex items-center justify-center hover:scale-105 transition-all shadow-[0_0_30px_rgba(0,229,255,0.4)]"
                              >
                                { (isPlayingPodcast || isPlayingAudiobook) ? <Pause size={32} fill="black" /> : <Play size={32} fill="black" className="ml-1" /> }
                              </button>
