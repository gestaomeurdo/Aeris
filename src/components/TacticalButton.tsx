...
const TacticalButton = ({ icon: Icon, label, onClick, className }: TacticalButtonProps) => {
  return (
    ...
    >
      {/* Indicator Light */}
      <div className="absolute top-2 right-2 w-1 h-1 rounded-full bg-zinc-800 group-hover:bg-amber-500 shadow-[0_0_5px_rgba(245,158,11,0.5)]" />
      
      <div className="p-2 rounded bg-black/40 border border-white/5 group-hover:border-amber-500/30 transition-colors">
        <Icon className="w-5 h-5 text-zinc-500 group-hover:text-amber-500 transition-colors" />
      </div>
      
      <span className="text-[9px] font-mono uppercase tracking-widest text-zinc-500 group-hover:text-amber-200 transition-colors">
        {label}
      </span>
