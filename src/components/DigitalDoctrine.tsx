...
import { Zap, Shield, Target, Cpu, ChevronRight, Lock } from 'lucide-react';

const modules = [
  { 
    id: "MOD-01", 
    title: "Fundamentos de Operações Urbanas", 
    desc: "Táticas de aproximação, CQB e controle de perímetro em ambientes de alta densidade.",
    icon: Target,
    progress: 85,
    locked: false,
  ...
  { 
    id: "MOD-02", 
    title: "Guerra Eletrônica e SIGINT", 
    desc: "Identificação de sinais, jamming tático e interceptação de comunicações inimigas.",
    icon: Cpu,
    progress: 30,
    locked: false,
  ...
  { 
    id: "MOD-03", 
    title: "Defesa Cibernética de Infraestrutura", 
    desc: "Proteção de grids de energia e sistemas críticos contra ataques persistentes.",
    icon: Shield,
    progress: 0,
    locked: true,
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {modules.map((mod, i) => (
          ...
          >
            {/* Background Glow */}
            {!mod.locked && (
              <div className={`absolute -top-24 -right-24 w-48 h-48 bg-gradient-to-br ${mod.color} opacity-[0.03] group-hover:opacity-[0.1] blur-3xl rounded-full transition-opacity`} />
            )}
            ...
            <div className="flex justify-between items-start">
              <div className={`p-4 rounded-2xl bg-white/5 border border-white/5 group-hover:border-[#00E5FF]/20 transition-all ${!mod.locked && 'group-hover:shadow-[0_0_20px_rgba(0,229,255,0.2)]'}`}>
                <mod.icon className={`w-8 h-8 ${mod.locked ? 'text-white/20' : 'text-[#00E5FF]'}`} />
              </div>
            ...
            <div className="mt-auto space-y-6">
              <div className="space-y-2">
                ...
                <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                  <motion.div 
                    ...
                    className={`h-full bg-gradient-to-r ${mod.color} shadow-[0_0_10px_rgba(0,229,255,0.3)]`}
                  />
                </div>
              </div>
              ...
              {mod.locked ? (
                ...
                <button className="group/btn relative w-full py-4 bg-white/5 rounded-2xl border border-white/10 flex items-center justify-center gap-3 hover:bg-[#00E5FF] transition-all duration-500 overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-r from-[#00E5FF] to-[#6366F1] translate-y-full group-hover/btn:translate-y-0 transition-transform duration-500" />
                </button>
              )}
            </div>
          </motion.div>
